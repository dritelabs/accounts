...
export const cors =
  ...
  (event: CompatibilityEvent) => {
    ...
    if (exposeHeaders.length) {
      event.res.setHeader('Access-Control-Expose-Headers', exposeHeaders.join(','));
    }

    const preFlight = event.req.method === 'OPTIONS';
    if (preFlight) {
      event.res.setHeader('Access-Control-Allow-Methods', allowMethods.join(','));
      event.res.setHeader('Access-Control-Allow-Headers', allowHeaders.join(','));
