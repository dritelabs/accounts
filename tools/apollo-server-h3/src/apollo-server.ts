...
import { CompatibilityEvent, send } from 'h3';
...
export class ApolloServer extends ApolloServerBase {
  ...
  async createGraphQLServerOptions(event: CompatibilityEvent): Promise<GraphQLOptions> {
  ...
  public createHandler({
    ...
    return async (event: CompatibilityEvent) => {
      ...
      try {
        ...
        if (landingPage && event.req.method === 'GET' && url == this.graphqlPath) {
  ...
  private async handleHealthCheck({
    ...
    if (!disableHealthCheck && event.req.url === '/.well-known/apollo/server-health') {
      // Response follows
  ...
  private async handleGraphqlRequestsWithLandingPage({
    ...
    const prefersHtml =
      types.find((x: string) => x === 'text/html' || x === 'application/json' || x === '*/*') === 'text/html';
  ...
  private async handleGraphqlRequestsWithServer({ event }: { event: CompatibilityEvent }) {
