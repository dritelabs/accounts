...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    authorizationServerHost: process.env.AUTHORIZATION_SERVER_HOST || 'http://localhost:3000',
    authorizationEndpoint: process.env.AUTHORIZATION_ENDPOINT || 'http://localhost:3000/authorize',
    clientId: process.env.CLIENT_ID || 'cl05amov600137o9kit49f4cz',
    clientAssertionExpirationTime: process.env.CLIENT_ASSERTION_EXPIRATION_TIME || '60',
    codeChallengeMethod: process.env.CODE_CHALLENGE_METHOD || 'S256',
    host: process.env.HOST || `http://localhost:${port}`,
    redirectUri: process.env.REDIRECT_URI || `http://localhost:${port}/api/callback`,
    ...
    tokenEndpoint: process.env.TOKEN_ENDPOINT || 'http://localhost:3000/api/token',
    privateKey,
    publicKey
  },
  typescript: {
