...
import { validateRefreshTokenGrantRequest, validateTokenRequest, idTokenClaimsReducer } from '../utils';
...
export const refreshToken: AccountHandlers['RefreshToken'] = async (call, callback) => {
  try {
    ...
    const refreshTokenGrantRequestValidation = await validateRefreshTokenGrantRequest(call.request);
    ...
    if (tokenRequestValidation.client.refreshTokenRotationType === 'rotate') {
