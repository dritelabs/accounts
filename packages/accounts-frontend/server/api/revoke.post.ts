...
export default withIronSession(async (event) => {
  try {
    ...
    const request = Object.fromEntries(params) as tokenService.TokenRequest &
      tokenService.RevocationRequest;
    const validation = await tokenService.validateRevocationRequest(request);

    const isAuthenticated =
      !!event.req.headers.authorization || !!request?.client_assertion;
