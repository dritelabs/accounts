...
const scopeNames = route.query?.scope
  ? (route.query.scope as string).split(" ")
  : [];

const validation = await validateAuthorizationRequest(
  route.query as AuthorizationRequest
);

const { data: client, error: clientError } = await useClient(
...
<template>
  ...
  >
    ...
    <div
      class="container"
      v-else-if="!client.redirectUris.includes(validation.redirect_uri)"
    >
    ...
    <div
      class="container"
      v-else-if="
      	!scopeNames.length || scopeNames.length !== scopes.items?.length
      "
    >
      ...
      <div class="columns is-centered">
        <div class="column is-5">
          ...
          <p class="subtitle has-text-centered">
            wants access to your Driten account:
          </p>
          <div
            class="is-flex is-align-items-center is-justify-content-center mb-5"
          >
          ...
          <p class="mb-5">
            ...
            in accordance with the respective terms of service and privacy
            policies. you can change this and other Account Permissions at any
            time.
          ...
          <form id="form-allow" method="post" action="/api/authorize/allow">
            ...
            />
            <select
              v-for="(value, key) in arrayValues"
              :name="key"
              multiple
              hidden
            >
            ...
            <button
              type="submit"
              class="button is-dark is-medium is-fullwidth mb-3"
            >
              Allow
            </button>
          </form>
